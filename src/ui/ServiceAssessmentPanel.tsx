...
export function ServiceAssessmentPanel({
  ...
  const onRun = async () => {
    ...
    setMessage(
      count
        ? `Assessment saved. ${count} misconfiguration issue(s) found.`
  ...
  return (
    <section className="app-card mt-6">
      ...
      <p className="text-sm text-health-subtle">
        Fixed Nmap check of common clinic ports plus TLS ciphers, certificate
        expiry, and SMB dialects/signing. Badges use NIST-style colors and a
        0–10 risk score. These are configuration facts, not exploits.
      </p>
      {canAssess ? (
        <BusyButton
